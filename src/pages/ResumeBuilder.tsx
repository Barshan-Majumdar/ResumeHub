...
`;
  ...
  const addExperience = () => {
    setExperiences([...experiences, { id: experiences.length + 1, company: "", position: "", duration: "", description: "" }]);
  };
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <header className="p-6 border-b border-primary/20">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Button variant="ghost" onClick={() => navigate("/templates")} className="text-foreground hover:text-primary">
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
          ...
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="space-y-6">
            ...
            <Card className="bg-gradient-card backdrop-blur-sm border border-primary/20">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input value={personalInfo.fullName} onChange={(e) => setPersonalInfo({ ...personalInfo, fullName: e.target.value })} placeholder="Your Name" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label className="text-foreground">Email</Label>
                    <Input value={personalInfo.email} onChange={(e) => setPersonalInfo({ ...personalInfo, email: e.target.value })} placeholder="you@email.com" />
                  </div>
                  <div>
                    <Label className="text-foreground">Phone</Label>
                    <Input value={personalInfo.phone} onChange={(e) => setPersonalInfo({ ...personalInfo, phone: e.target.value })} placeholder="123-456-7890" />
                  </div>
                </div>
                <div>
                  <Label className="text-foreground">Address</Label>
                  <Input value={personalInfo.address} onChange={(e) => setPersonalInfo({ ...personalInfo, address: e.target.value })} placeholder="City, Country" />
                </div>
                <div>
                  <Label className="text-foreground">Professional Summary</Label>
                  <Textarea value={personalInfo.summary} onChange={(e) => setPersonalInfo({ ...personalInfo, summary: e.target.value })} placeholder="A brief summary about yourself..." rows={3} />
                </div>
              </CardContent>
            </Card>
            {/* Experience */}
                  ...
                  <div key={exp.id} className="border border-primary/10 rounded-lg p-4 space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      {experiences.length > 1 && (
                        <Button onClick={() => removeExperience(exp.id)} size="sm" variant="ghost" className="text-destructive hover:text-destructive">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <Input value={exp.duration} onChange={(e) => updateExperience(exp.id, "duration", e.target.value)} placeholder="2020 - 2023" />
                      </div>
                    </div>
                    <div>
                ...
                {education.map((edu) => (
                  <div key={edu.id} className="border border-primary/10 rounded-lg p-4 space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      {education.length > 1 && (
                        <Button onClick={() => removeEducation(edu.id)} size="sm" variant="ghost" className="text-destructive hover:text-destructive">
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <Input value={edu.degree} onChange={(e) => updateEducation(edu.id, "degree", e.target.value)} placeholder="Bachelor's in Computer Science" />
                      </div>
                      <div>
            ...
            <Card className="bg-gradient-card backdrop-blur-sm border border-primary/20">
              ...
              <CardContent>
                ...
                <Textarea id="skills" value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="JavaScript, React, Node.js, Python..." rows={3} />
              </CardContent>
            </Card>
          </motion.div>
        </div>
        ...
        <div className="lg:w-1/2 p-6 bg-background/50">
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="sticky top-6">
                ...
                <CardContent className="p-8">
                  <div className="space-y-6">
                    ...
                    <div className="text-center border-b pb-4">
                      <h1 className={`text-2xl font-bold ${selectedStyle.header}`}>{personalInfo.fullName || "Your Name"}</h1>
